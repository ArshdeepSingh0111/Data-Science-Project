/* Adapted from "Project final draft code.sas" (ArshdeepSingh0111/Data-Science-Project).
   ...
   nine columns (cement, slag, fly ash, water, superplasticizer, coarse/fine aggregate,
   age, compressive strength) -- the public UCI Concrete Compressive Strength schema
   the original script analyzes. Everything below the data step is the author's own
   logic, unmodified: the age-quartile flag (Q) and the log-transform (lop), then the
   PROC UNIVARIATE normality check on both the raw and log-transformed strength. */
