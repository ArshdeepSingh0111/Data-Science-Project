/* Adapted from "Project final draft code.sas" (ArshdeepSingh0111/Data-Science-Project).
   ...
   author's own code, unmodified: the age-quartile flag (Q), and their two PROC REG
   calls -- the full model with /vif, and the same model with selection=backward --
   regressing compressive strength on the seven mix components plus Q. */
